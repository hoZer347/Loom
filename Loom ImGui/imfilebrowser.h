...
inline void ImGui::FileBrowser::Display()
{
    ...
    if(SmallButton("*"))
    {
        ...
        for(auto &m_name : selectedFilenames_)
        {
            const auto it = std::find_if(
                fileRecords_.begin(), fileRecords_.end(), [&](const FileRecord &record)
                {
                    return m_name == record.m_name;
                });
            if(it != fileRecords_.end())
            {
                newSelectedFilenames.insert(m_name);
            }
        }
...

    {
        ...
        for(unsigned int rscIndex = 0; rscIndex < fileRecords_.size(); ++rscIndex)
        {
            ...
            if(!rsc.m_name.empty() && rsc.m_name.c_str()[0] == '$')
            {
                continue;
            }

            const bool selected = selectedFilenames_.find(rsc.m_name) != selectedFilenames_.end();
            if(Selectable(rsc.showName.c_str(), selected, ImGuiSelectableFlags_DontClosePopups))
            {
                ...
                const bool canSelect = rsc.m_name != ".." && rsc.isDir == wantDir;
                ...
                if(rangeSelect)
                {
                    ...
                    for(unsigned int i = first; i <= last; ++i)
                    {
                        ...
                        selectedFilenames_.insert(fileRecords_[i].m_name);
                    }
                }
                else if(selected)
                {
                    if(!multiSelect)
                    {
                        selectedFilenames_ = { rsc.m_name };
                        rangeSelectionStart_ = rscIndex;
                    }
                    else
                    {
                        selectedFilenames_.erase(rsc.m_name);
                    }
                    if(flags_ & ImGuiFileBrowserFlags_EnterNewFilename)
                    {
                ...
                else if(canSelect)
                {
                    if(multiSelect)
                    {
                        selectedFilenames_.insert(rsc.m_name);
                    }
                    else
                    {
                        selectedFilenames_ = { rsc.m_name };
                    ...
                    if(flags_ & ImGuiFileBrowserFlags_EnterNewFilename)
                    {
                        const auto rscName = u8StrToStr(rsc.m_name.u8string());
            ...
            if(IsItemClicked(0) && IsMouseDoubleClicked(0))
            {
                ...
                {
                    shouldSetNewDir = true;
                    newDir = (rsc.m_name != "..") ? (currentDirectory_ / rsc.m_name) : currentDirectory_.parent_path();
                }
    ...
    if(!focusOnInputText && !editDir_)
    {
        ...
        if(selectAll)
        {
            ...
            for(size_t i = 1; i < fileRecords_.size(); ++i)
            {
                ...
                {
                    selectedFilenames_.insert(record.m_name);
                }
            }
        }
    }
...
inline void ImGui::FileBrowser::UpdateFileRecords()
{
    ...
    std::sort(
        ...
        {
            return (L.isDir ^ R.isDir) ? L.isDir : (L.m_name < R.m_name);
        });
