...
bool    ImGui::BeginTableEx(const char* m_name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (use_child_window)
    {
        ...
        BeginChildEx(m_name, instance_id, outer_rect.GetSize(), false, child_flags);
...
void ImGui::TableHeadersRow()
{
    ...
    for (int column_n = 0; column_n < columns_count; column_n++)
    {
        ...
        const char* m_name = (TableGetColumnFlags(column_n) & ImGuiTableColumnFlags_NoHeaderLabel) ? "" : TableGetColumnName(column_n);
        PushID(column_n);
        TableHeader(m_name);
        PopID();
    }
...
void ImGui::TableOpenContextMenu(int column_n)
{
    ...
    if (table->Flags & (ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable))
    {
        ...
        table->InstanceInteracted = table->InstanceCurrent;
        const ImGuiID context_menu_id = ImHashStr("##ContextMenu", 0, table->m_ID);
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display)
{
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
    {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            ...
            const char* m_name = TableGetColumnName(table, other_column_n);
            ...
            if (MenuItem(m_name, NULL, other_column->IsUserEnabled, menu_item_active))
...
ImGuiTableSettings* ImGui::TableGetBoundSettings(ImGuiTable* table)
{
    if (table->SettingsOffset != -1)
    {
        ...
        IM_ASSERT(settings->m_ID == table->m_ID);
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    if (settings == NULL)
    {
        settings = TableSettingsCreate(table->m_ID, table->ColumnsCount);
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    if (table->SettingsOffset == -1)
    {
        settings = TableSettingsFindByID(table->m_ID);
        if (settings == NULL)
            return;
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        if (settings->m_ID == 0) // Skip ditched settings
            continue;
        ...
        buf->appendf("[%s][0x%08X,%d]\n", handler->TypeName, settings->m_ID, settings->ColumnsCount);
...
void ImGui::TableRemove(ImGuiTable* table)
{
    ...
    g.Tables.Remove(table->m_ID, table);
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    bool open = TreeNode(table, "Table 0x%08X (%d columns, in '%s')%s", table->m_ID, table->ColumnsCount, table->OuterWindow->Name, is_active ? "" : " *Inactive*");
    ...
    if (g.IO.ConfigDebugIsDebuggerPresent)
    {
        if (DebugBreakButton("**DebugBreak**", "in BeginTable()"))
            g.DebugBreakInTable = table->m_ID;
        SameLine();
    }
    ...
    {
        ImGuiTableColumn* column = &table->Columns[n];
        const char* m_name = TableGetColumnName(table, n);
        char buf[512];
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            n, column->DisplayOrder, m_name, column->MinX - table->WorkRect.Min.x, column->MaxX - table->WorkRect.Min.x, (n < table->FreezeColumnsRequest) ? " (Frozen)" : "",
...
{
    if (!TreeNode((void*)(intptr_t)settings->m_ID, "Settings 0x%08X (%d columns)", settings->m_ID, settings->ColumnsCount))
        return;
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    IM_ASSERT(g.ActiveId == columns->m_ID + ImGuiID(column_index));
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            const ImGuiID column_id = columns->m_ID + ImGuiID(n);
